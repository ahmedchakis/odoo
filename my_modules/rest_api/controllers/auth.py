...
def define_token_expires_in(token_type, jdata):
...
# New function _get_token
def _get_token(signUp=False, **jdata):
    response = {
        'success': False,
        'responseCode': 401,
        'message': "Odoo User authentication failed!"
    }
    username = jdata.get('login')
    password = jdata.get('password')
    # Empty 'username' or 'password:
    if not username or not password:

        response['message'] = "Empty value of 'login' or 'password'!"
        return response
    ...
    if not signUp:
        ...
        # Odoo login failed:
        if not uid:
            response['message'] = "Odoo User authentication failed!"
            return response

    else:
        uid = jdata.get('user_id', "")
    # Generate tokens
    access_token = generate_token()
    expires_in = define_token_expires_in('access', jdata)
    refresh_token = generate_token()
    refresh_expires_in = define_token_expires_in('refresh', jdata)
    ...
    response = {'success': True, 'responseCode': 1, 'message': "Created Successfully.", 'token': {
        'access_token': access_token,
        'expires_in': expires_in,
        'refresh_token': refresh_token,
        'refresh_expires_in': refresh_expires_in
